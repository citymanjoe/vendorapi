...
public class FintrakServiceImpl implements FintrakService {
	...
	public ResponseEntity<?> getCustomerTransactionDetail(String accountNo, Date startDate, Date currentDate) {
		try {
			return new ResponseEntity<>(new SuccessResponse("CUSTOMER TRANSACTION DETAIL.",
					finDAO.GetCustTransDetail(accountNo, startDate, currentDate)), HttpStatus.OK);
		} catch (Exception e) {
	...
	public ResponseEntity<?> getCustomerLoanInterestDetail(String accountNo, int monthNo) {
		try {
			return new ResponseEntity<>(
					new SuccessResponse("CUSTOMER INTEREST DETAIL.", finDAO.GetCustLoanIntDetail(accountNo, monthNo)),
					HttpStatus.OK);
		} catch (Exception e) {
			return new ResponseEntity<>(new ErrorResponse(e.getLocalizedMessage()), HttpStatus.BAD_REQUEST);
		}
